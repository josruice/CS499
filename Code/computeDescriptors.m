...
%   cell array where the i,j cell contains a matrix with the descriptors by 
%   columns of the image of with file path in the ith row and jth column of the 
...
[nFolders, nFilesPerFolder] = size(filePathsCellArray);
descriptorsCellArray = cell(nFolders, nFilesPerFolder);
...
for iFolder = 1:nFolders,
    for jFileName = 1:nFilesPerFolder,
        ...
        % Load the image.
        try
            filePath = filePathsCellArray{iFolder, jFileName};
        ...
        descriptorsCellArray{iFolder,jFileName} = D;
    end
end